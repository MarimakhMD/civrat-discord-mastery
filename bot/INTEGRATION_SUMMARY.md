...
Les dashboards actuels envoient `expectedUpdatedAt` (`string` ISO de la ligne chargée, ou `null` si elle n’existe pas encore) à `PUT /api/guilds/:guildId/config`. Pendant un déploiement progressif, l’API accepte encore un ancien client sans ce champ et récupère alors une version fraîche côté serveur. Le service effectue un `UPDATE ... WHERE guild_id = ... AND updated_at = expectedUpdatedAt`; l’insertion initiale est protégée par la contrainte unique `guild_id`. Si une autre session a enregistré entre-temps, l’API répond `409` sans écrire ni recharger le cache. Cette protection optimiste évite l’écrasement silencieux entre onglets, sans migration destructive ni nouvelle table.
